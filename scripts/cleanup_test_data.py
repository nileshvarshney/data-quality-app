...
async def cleanup():
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(text("""
            ...
            WHERE sf_schema_name = :schema AND sf_table_name = :table
        ...
        result = await db.execute(text("""
            SELECT rule_id FROM dq_rules WHERE asset_id = ANY(:ids)
        """), {"ids": asset_ids})
        ...
        if rule_ids:
            result = await db.execute(text("""
                SELECT run_id FROM dq_rule_runs WHERE rule_id = ANY(:ids)
            """), {"ids": rule_ids})
        ...
        if run_ids:
            r = await db.execute(text(
                "DELETE FROM dq_rule_run_samples WHERE run_id = ANY(:ids)"
            ), {"ids": run_ids})
        ...
        if run_ids:
            r = await db.execute(text(
                "DELETE FROM dq_alerts WHERE run_id = ANY(:ids)"
            ), {"ids": run_ids})
            print(f"  Deleted {r.rowcount} alert(s) by run_id")

        if rule_ids:
            r = await db.execute(text(
                "DELETE FROM dq_alerts WHERE rule_id = ANY(:ids)"
            ), {"ids": rule_ids})
            print(f"  Deleted {r.rowcount} alert(s) by rule_id")

        if asset_ids:
            r = await db.execute(text(
                "DELETE FROM dq_alerts WHERE asset_id = ANY(:ids)"
            ), {"ids": asset_ids})
        ...
        if run_ids:
            r = await db.execute(text(
                "DELETE FROM dq_rule_runs WHERE run_id = ANY(:ids)"
            ), {"ids": run_ids})
        ...
        if asset_ids:
            r = await db.execute(text(
                "DELETE FROM dq_quality_scores WHERE asset_id = ANY(:ids)"
            ), {"ids": asset_ids})
        ...
        if rule_ids:
            r = await db.execute(text(
                "DELETE FROM rule_versions WHERE rule_id = ANY(:ids)"
            ), {"ids": rule_ids})
        ...
        if rule_ids:
            r = await db.execute(text(
                "DELETE FROM rule_tags WHERE rule_id = ANY(:ids)"
            ), {"ids": rule_ids})
        ...
        if rule_ids:
            r = await db.execute(text(
                "DELETE FROM dq_schedules WHERE rule_id = ANY(:ids)"
            ), {"ids": rule_ids})
            print(f"  Deleted {r.rowcount} schedule(s) by rule_id")

        if asset_ids:
            r = await db.execute(text(
                "DELETE FROM dq_schedules WHERE asset_id = ANY(:ids)"
            ), {"ids": asset_ids})
        ...
        if all_entity_ids:
            r = await db.execute(text(
                "DELETE FROM sla_configs WHERE entity_id = ANY(:ids)"
            ), {"ids": all_entity_ids})
        ...
        if rule_ids:
            r = await db.execute(text(
                "DELETE FROM audit_logs WHERE entity_id = ANY(:ids)"
            ), {"ids": rule_ids})
        ...
        if rule_ids:
            r = await db.execute(text(
                "DELETE FROM dq_rules WHERE rule_id = ANY(:ids)"
            ), {"ids": rule_ids})
            print(f"  Deleted {r.rowcount} rule(s)")

        # 4k. data_assets  (the asset itself)
        r = await db.execute(text(
            "DELETE FROM data_assets WHERE asset_id = ANY(:ids)"
        ), {"ids": asset_ids})
